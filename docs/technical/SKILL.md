...
`buildDocument`는 아래 7개 빌더를 순서대로 조립한다(각 `safeSection`으로 장애 격리).
...
| 6 | 참고 — 목록에서 찾는 방법 | `buildListingNotice` (safeSection `"listing"`) | `listedOutOfOrder` 건 ≥1 | "참고 — 금감원 목록에서 찾는 방법" + 해당 건별 **조치요구일 · 최초 등장일(`firstSeenDate`) · 목록 행 번호(`listRank`)** |
| 7 | 오늘 하나만 | `buildClosing` | graded≥1 | 구분선(section) + "오늘 하나만 기억하세요." + 한 줄 |

> **버려진 빌더:** `buildOtherItems`("그 외 새로 확인된 제재·경영유의")는 코드에 정의돼 있으나 `buildDocument`에서 **호출하지 않는다**. 모든 항목은 §3 카드로 전건 수록되므로 별도 "그 외" 목록은 출력되지 않는다.
...
| 분기 | 조건 | 문구 |
|------|------|------|
| noUpdate | `data.noUpdate` | "금융감독원에서 새로 확인된 제재·경영유의가 없어요." + "지난 실행 이후 새로 게시된 건이 없어요. 기존 진행건 점검을 유지해 주세요." |
| 빈 상태 | graded 0건 | "새로 확인된 건 중 IBK 연관 제재·경영유의는 없어요." + "기존 점검 체계를 재점검하는 시간으로 활용해보세요 🙂" |
| 일반 | graded≥1 | "금융감독원에서 새로 확인된 제재·경영유의는 {N}건이에요." + "그 중 지금 바로 살펴봐야 할 건 {상 등급 수}건이에요." (상≥1이면 그 수를 빨강 강조) |
...
{계층라벨}  ·  {제재조치일·조치요구일}  ·  {제재유형}   (메타 줄, TS.caption 18 gray1 — 값 있는 것만 "  ·  "로 연결)
...
- **참고 — 금감원 목록에서 찾는 방법:** FSS 목록엔 게시일 컬럼이 없고 제재조치요구일 내림차순으로 정렬돼, 새로 게시된 건도 조치요구일이 과거면 목록 중간에 삽입된다. 수신자 혼동을 막기 위해 `listedOutOfOrder`(조치요구일 < 최초 등장일) 건에 한해 **조치요구일 · 최초 등장일 · 목록 행 번호**를 안내한다(공용 `listingNoticeLines()`). 해당 건이 없으면 섹션 자체를 싣지 않는다(소음 방지).
...
```
🔴|🔶|🔹 제재대상: {기관명} [{계층라벨|기관}] · {제재조치일·조치요구일} · {제재유형}
...
**꼬리말 — '목록에서 찾는 법' 안내(`listingNoticeLines`, 알림 대상에 해당 건이 있을 때만):** `alertItems` 중 `listedOutOfOrder`(조치요구일 < 최초 등장일)인 건이 하나라도 있으면 tgMsg **말미**에 붙는다. 해당 건이 없으면 아예 붙이지 않는다(소음 방지).
```
ℹ️ 참고 — 금감원 목록에서 찾는 방법
금감원 공시 목록은 게시된 순서가 아니라 제재조치요구일 순으로 정렬돼요. …
맨 윗줄에 안 보인다고 해서 잘못된 알림은 아니에요.
• {기관명} — 조치요구일 {YYYY-MM-DD} · {최초 등장일} 목록에 처음 등장 · 확인 시점 목록 {N}번째 줄
```
...
_last updated: 2026-07-12 (buildListingNotice 7번째 빌더 · tgMsg 목록 안내 꼬리말 반영)_
