...
**중복방지 ledger (`state/seen_ids.json`):**
발행이 부정기적이므로 이미 본 건을 재알림하지 않도록 **`state/seen_ids.json`** 을 유일한 상태 저장소로 둔다. 클라우드 실행이라 repo가 곧 상태 저장소이며, **성공 시에만** ledger를 커밋해 중복방지 상태를 지속한다. 최초 실행(ledger 빔)은 **시드 모드**로 과거건을 보고 대상에서 제외하고 ledger만 채워 초기 범람을 막는다.
...
**tgMsg 구조** — 헤더 `🔔 FSS 제재·경영유의 브리핑 (HH:MM)` + 알림 대상(T0~T2) 각 건을 다음 **질문형 라벨 + 질문·답변 2계층**으로 표시한다.
