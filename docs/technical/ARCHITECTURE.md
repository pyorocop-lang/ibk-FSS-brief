...
GitHub Actions의 schedule cron은 최대 약 ~12시간의 지연·누락이 확인되어 **제거**했다(백업으로도 두지 않음). 정시성은 외부 Cloudflare Workers Cron이 전담한다.
...
**목록 정렬 특성 (신규 판정의 전제):**
**FSS 목록엔 게시일 컬럼이 없다.** 3번째 컬럼은 `제재조치요구일`(`actionRequestDate`)이고 목록은 그 값의 **내림차순 정렬**이다. 따라서 오늘 새로 게시된 건도 조치요구일이 과거면 **목록 맨 위가 아니라 중간에 삽입**된다 → **조치요구일(날짜)로는 신규를 판정할 수 없다.**

**신규 판별 — 직전 실행 관측 창 차집합(scan-window diff) + 중복방지 ledger (`state/seen_ids.json`):**
직전 실행의 `crawl_result.scanAudit`(페이지별 전체 행 key + 훑은 깊이)을 복원해(`buildScanWindow`), **그 깊이 안에서 그때는 없다가 지금 나타난 행**만 신규로 본다. `classifyRow(key, page, ledgerMap, win, seed)`가 3분기로 판정한다.

| 판정 | 조건 | 처리 |
|---|---|---|
| `known` | 레저에 있거나 직전 창에 있음 | 재알림 차단 |
| `new` | 직전 창 **깊이 안**에서 새로 나타남 | 신규 보고 (조치요구일이 과거여도 신규) |
| `backfill` | 최초 시드(seed) · 직전 창 **밖 깊이**(판정 근거 없음) | 레저에만 등록 + 보고 제외 |

`backfill`은 레저에만 등록하고 보고에서 제외하되 `crawl_result.backfilled[]`에 **명시 기록**한다 — 침묵 폐기(조용히 버리기)를 하지 않는다. 창 밖 깊이를 backfill로 격리하므로 `--pages` 확장 시 과거 누적분이 범람하지 않는다. 직전 창이 유실되면 `WINDOW_FALLBACK_DEPTH` 깊이를 가정해 레저로만 판정한다. 산출: `crawl_result.{ newItems[], backfilled[], scanWindow{}, completeness{}, scanAudit[] }`, `newItemBasis: "scan-window-diff"`. 레저 `state/seen_ids.json`은 유일한 상태 저장소로 **성공 시에만** 커밋해 중복 알림(특히 08:00·16:00 두 실행 간)을 막는다.

> **폐지: `REPORT_SINCE`(게시일 앵커) — 2026-07-10.** 정렬 컬럼(조치요구일)에 커트오프를 걸어 늦게 게시된 과거 조치요구일 건을 침묵 폐기했다(실제 사고: `아이비케이신용정보` — 2026-07-09 신규 게시, 조치요구일 06-25). env로 설정해도 **무시**되고 경고만 출력된다. 필드명도 `postDate`(오칭) → **`actionRequestDate`**로 정정됐다.
...
**'목록에서 찾는 법' 안내:** 신규 건이 목록 상단에 없어 수신자가 혼동하므로, `listedOutOfOrder`(조치요구일 < 최초 등장일)인 건에 한해 **조치요구일 · 최초 등장일(`firstSeenDate`) · 목록 행 번호(`listRank`)** 를 안내한다(briefV2 `listingNoticeLines()` → DOCX `buildListingNotice` + Telegram `tgMsg` 말미). 해당 건이 없으면 안내를 아예 싣지 않는다.
...
flowchart TD
    ...
    subgraph STEP1["STEP1 수집+dedup (클라우드)"]
        C1["목록 조회 (menuNo 200476 / 200483)\n앵커 href에서 상세경로 추출\n※ 게시일 컬럼 없음 · 조치요구일 desc 정렬"]
        C2["신규 판정 — 관측 창 차집합(scan-window diff)\n직전 scanAudit 창 복원 → classifyRow\nknown / new / backfill · state/seen_ids.json 대조"]
        C3["상세·PDF 본문 추출(신규만)\n+ Tier 분류 · 등급 산정"]
        C4["crawl_result.json\nnewItems·backfilled·scanWindow·scanAudit\nreports/DATE/SLOT/ (+ raw·pdfs)"]
    end
    ...
    subgraph STEP3["STEP3 보고서"]
        ...
        B2b["참고 — 금감원 목록에서 찾는 방법\nbuildListingNotice (listedOutOfOrder 건만)"]
        B3["tgMsg 생성(질문형 2계층 + 목록에서 찾는 법 꼬리말)"]
    end
...
| 제약 | 원인 | 대응 |
|---|---|---|
| GitHub schedule cron 정시성 부족 | 최대 ~12h 지연·누락 | Cloudflare Workers Cron으로 08:00 KST(`0 23 * * *`)·16:00 KST(`0 7 * * *`) 2회 트리거 |
| 발행 부정기 → 과거 누적 공시가 '당일 신규'로 오인 | 목록에 과거 공시 누적 노출 · 조치요구일 desc 정렬(게시일 컬럼 없음) | **관측 창 차집합(scan-window diff)** + `state/seen_ids.json` dedup ledger(성공 시에만 커밋). 창 밖·최초 시드는 `backfill`로 격리(보고 제외·명시 기록), 창 유실 시 `WINDOW_FALLBACK_DEPTH` 가정. 관측 창 깊이 `--pages` 기본 5 |
