...
- **게시일 앵커(고정): 게시일(postDate) ≥ `REPORT_SINCE`(기본 2026-07-03)인 건만 보고.** 앵커 이전 게시분은 '백로그' — 레저에만 등록해 재검토를 막고 알림·보고에선 완전 제외(상세수집도 생략). 게시일 파싱 실패는 fail-open(보고)해 실제 건을 놓치지 않는다. 앵커는 env `REPORT_SINCE`로 재정의 가능.
