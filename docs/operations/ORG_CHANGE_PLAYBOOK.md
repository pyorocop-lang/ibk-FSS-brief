...
2. `org:validate`로 ID·명칭·계층·시행일·출처·증거등급을 검증한다.
3. `org:generate`로 사람이 읽는 레지스트리를 갱신한다.
4. `org:audit`로 코드·프롬프트·과거 분석의 비현행 배정을 탐지한다.
5. 테스트와 GitHub Actions가 모두 통과해야 병합한다.
