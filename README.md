...
> ✅ **라이브 운영 중** — 매일 08:00·16:00 KST 완전 클라우드 자동 실행.
...
FSS OPEN API에는 제재/경영유의 엔드포인트가 없어 HTML/PDF 직접 스크래핑을 채택. FSS는 해외 IP 차단이 없어(미국 러너 접근 PASS) 프록시 없이 GitHub Actions에서 직결 수집. FSS 목록에는 **'게시일' 칸이 없고** 제재조치요구일 순으로 정렬돼, 오늘 새로 올라온 건도 조치요구일이 과거면 목록 중간에 꽂힌다 → 날짜로는 신규를 가릴 수 없다. 그래서 **직전 실행이 실제로 확인한 목록(관측 창)과 대조해 그 사이 새로 나타난 건만** 신규로 보고하고(**관측 창 차집합**), `state/seen_ids.json` 중복방지 레저가 이미 본 건의 재알림을 막는다. 직전에 훑지 않은 깊이(창 밖)와 최초 실행분은 과거 누적분이므로 레저에만 등록하고 보고에서 제외하되, 조용히 버리지 않고 `backfilled[]`에 기록으로 남긴다.
...
Cloudflare Workers Cron (매일 08:00·16:00 KST = 23:00·07:00 UTC, cron "0 23 * * *"·"0 7 * * *")
  → GitHub workflow_dispatch (.github/workflows/daily-brief.yml, 단일 Job / 실행당 1 Job)
      ...
      fss_crawler.js (2소스 스크래핑 + 관측 창 차집합 신규 판정(+seen_ids 레저))
