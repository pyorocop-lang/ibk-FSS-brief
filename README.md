...
FSS OPEN API에는 제재/경영유의 엔드포인트가 없어 HTML/PDF 직접 스크래핑을 채택. FSS는 해외 IP 차단이 없어(미국 러너 접근 PASS) 프록시 없이 GitHub Actions에서 직결 수집. 누적 목록에서 신규만 가리기 위해 **게시일 앵커(REPORT_SINCE, 기본 2026-07-02) + `state/seen_ids.json` dedup ledger**를 병행 — 게시일이 앵커 이상 AND 레저에 없던 건만 보고하고, 앵커 이전 게시분은 백로그로 제외한다.
