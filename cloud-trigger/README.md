...
```
[Cloudflare Workers Cron 08:00·16:00 KST 매일] --POST workflow_dispatch--> [GitHub Actions: 수집+dedup→분석→보고서→검증→알림]
```
...
wrangler deploy
```
> Cron 스케줄(`0 23 * * *` = 08:00 KST, `0 7 * * *` = 16:00 KST, 매일)은 wrangler 쓰기가 막혀 있어
> Cloudflare 대시보드 → Workers → Triggers 에서 **두 cron을 직접 추가/확인**한다.
> ※ 주말 신규 0건은 조용한 알림이라 무해.
...
## 비고
- 비용: Cloudflare Workers 무료 플랜으로 충분 (cron 1일 2회, 매일).
