...
```
[Cloudflare Workers Cron 08:00 KST 매일] --POST workflow_dispatch--> [GitHub Actions: 수집+dedup→분석→보고서→검증→알림]
```
...
wrangler deploy
```
> Cron 스케줄(`0 23 * * *` = 08:00 KST 매일)은 wrangler 쓰기가 막혀 있어
> Cloudflare 대시보드 → Workers → Triggers 에서 직접 추가/확인한다.
> ※ 대시보드 드롭다운이 평일범위(0-4)를 못 받아 매일로 설정 — 주말 신규 0건은 조용한 알림이라 무해.
