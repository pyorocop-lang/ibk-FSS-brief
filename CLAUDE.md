...
매일 08:00 KST Cloudflare Workers Cron이 GitHub Actions를 호출 → 단일 Job이 수집→분석→보고서→검증→아카이브→알림을 수행.
전체 목적·소스·결정사항·아키텍처는 [PROJECT_BRIEF.md](PROJECT_BRIEF.md), 상세 파이프라인은 [workflow.md](workflow.md)·[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) 참조.

## 파이프라인 (완전 클라우드)
```
Cloudflare Workers Cron (매일 23:00 UTC = 08:00 KST, 대시보드 cron "0 23 * * *")
  → GitHub workflow_dispatch (.github/workflows/daily-brief.yml, 단일 Job)
      ...
      Artifact 업로드 + 완료 알림
```
- 산출물은 `reports/{DATE}/{SLOT}/` 에 런별 분리 보존(SLOT: 08:00=am / 수동 오후 재실행=pm, runslot.js). 08:00 정시 발화는 단일(am).
...
- **B. 실행 시각**: 08:00 KST. 대시보드 cron `0 23 * * *`(매일). ※ 드롭다운이 평일범위(0-4)를 못 받아 매일로 설정 — 주말 신규 0건은 조용한 알림이라 무해.
