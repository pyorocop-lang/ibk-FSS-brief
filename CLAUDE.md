...
매일 08:00·16:00 KST Cloudflare Workers Cron이 GitHub Actions를 호출 → 실행당 단일 Job이 수집→분석→보고서→검증→아카이브→알림을 수행(하루 2회, FSC 동형).
...
- 발행 부정기적 → **게시일 앵커(REPORT_SINCE, 기본 2026-07-02) + 중복방지 ledger(state/seen_ids.json)** 로 신규 판별: **게시일(postDate)이 앵커 이상 AND 레저에 없던 건**만 보고, 앵커 이전 게시분은 백로그로 레저 등록·보고 완전 제외(레저 부재만으론 과거 누적 공시가 '당일 신규'로 샘 — 총평단 2026-07-03 지적). 클라우드 실행이므로 repo가 유일한 상태 저장소.
